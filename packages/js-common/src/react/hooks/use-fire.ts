import { calculateAge, defineStatePensionAgeForGenderUK } from "../../utils/projection-utils";
...
import {
  ...
  FireSettingsFormValues,
  FireSettingsInsert,
  fireSettingsOrphanFormSchema,
  IncomeGoalKey,
} from "../../schema/portfolio-fire";
import type { IncomeGoal } from "../../schema/portfolio-fire";
import { createDecimalValueString } from "../../schema/decimal-value";
import { Contributor, FireProjection, FIREProjectionConfig, ProjectionConfig, ProjectionModifier, SimpleProjectionConfig } from "../../schema";
...
import { fireProjection as fireProjectionQueryKey } from "../../api/queryKeys";
import { useFIREProjection } from "./use-projections";
import { DecimalValueString } from "../../schema/decimal-value";
import { createRRulePattern } from "../../utils/scheduling";
...
export const useFireProjection = (): UseFireProjectionReturn => {
  ...
  const fireSettingsForm = useForm<FireSettingsFormValues>({
    ...
    values: fireSettings
      ? {
        annualIncomeGoal: fireSettings?.annualIncomeGoal ?? createDecimalValueString("0"),
        //expectedAnnualReturn: fireSettings?.expectedAnnualReturn ?? "7",
        safeWithdrawalRate: fireSettings?.safeWithdrawalRate ?? createDecimalValueString("4"),
      ...
      : undefined,
    defaultValues: {
      annualIncomeGoal: createDecimalValueString("0"),
