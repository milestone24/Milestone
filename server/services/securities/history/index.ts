import { SecurityHistory, SecurityIdentifier } from "../types";
...
export const getSecurityHistoryForIdentifierForDateRange = async (
  ...
  const cachedHistory = await getSecurityHistoryForDateRangeFromCache(
    security.id,
    startDate,
    endDate
  );
  ...
  const fillStartDate =
    cachedHistory.length > 0
      ? addDays(cachedHistory.at(-1)!.date, 1)
      : startDate;

  const historyFill = await getSecurityHistoryForDateRangeFromGateway(
    identifier,
    new Date(fillStartDate),
    endDate
  );
