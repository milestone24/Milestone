...
const RecordTransactionContext = createContext<RecordTransactionContextType | null>(null);

export const RecordTransactionProvider = ({
  children,
}: {
  children: ReactNode;
}) => {
  ...
  const [pageAssetId, setPageAssetId] = useState<string | undefined>(undefined);
  ...
  return (
    <RecordTransactionContext.Provider
      value={{ open, assetId, openTransaction, closeTransaction, setPageAssetId }}
    >
      {children}
    </RecordTransactionContext.Provider>
  );
