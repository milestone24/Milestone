...
EC2_HOST=$(aws ec2 describe-instances --filters 'Name=tag:Name,Values=MilestoneStack/MilestoneApp/Instance' |
...
echo "Streaming docker compose logs from ${EC2_REMOTE}..."
ssh -i ${EC2_SSH_KEY} ${EC2_REMOTE} "cd /opt/milestone && docker-compose logs -f"
