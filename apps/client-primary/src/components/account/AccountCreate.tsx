...
import {
  type AssetSecurityNewFormData,
  AssetSecurityNewFormFields,
} from "./AssetSecurityForm";
import { userAssetSecurityOrphanNewCreateInsertSchema } from "@milestone/js-common/schema/portfolio-assets";
import {
  RecurringContributionForm,
  type RecurringContributionFormProps,
} from "./RecurringContributionForm";
import { createRRulePattern } from "@milestone/js-common/utils/scheduling";
...
export const AccountCreate: React.FC<AccountCreateProps> = ({
  ...
  const stage1Fields: (keyof UserAssetOrphanInsert)[] = [
    "platformId",
    "accountType",
    "startDate",
  ];
  const stage2Fields: (keyof UserAssetOrphanInsert)[] = [
    "valueMethod",
    "securities",
    "currentValue",
  ];
  ...
  return (
    <>
      <div className="flex flex-row justify-between items-center">
        <h1 className="text-2xl font-bold">Add Investment Account</h1>
      </div>
      <Form {...form}>
        <form
        ...
        >
          {formStage === 1 && (
            <AccountCreateOne onNext={handleNext} onCancel={handleCancel} />
          )}
          {formStage === 2 && (
            <AccountCreateTwo
              onNext={handleNext}
...
const AccountCreateOne: React.FC<AccountCreateFormProps> = (props) => {
  ...
  const canNext =
    ...
    accountTypeFieldState.isDirty &&
    !startDateFieldState.invalid &&
    startDateFieldState.isDirty;
  ...
  const selectableAccountTypes = useMemo(() => {
    return (
      selectedPlatform ? selectedPlatform.supportedAccountTypes : accountType
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            ...
            >
              <FormControl>
                <SelectTrigger onBlur={field.onBlur} onChange={field.onChange}>
...
const AccountCreateThree: React.FC<AccountCreateFormProps> = (props) => {
  ...
  const contributionsAmountIsPositive = contributionsAmount
    ? Decimal(contributionsAmount).gt(0)
    : false;
