...
export default function MobileSettings() {
  ...
  useEffect(() => {
    const loadPreferences = async () => {
      ...
      const offlineMode = await getStoredValue<boolean>("offlineMode");
      if (offlineMode !== null) setEnableOfflineMode(offlineMode);

      const notifications = await getStoredValue<boolean>("notifications");
      if (notifications !== null) setEnableNotifications(notifications);

      const freshness = await getStoredValue<string>("dataFreshness");
      if (freshness !== null) setDataFreshness(freshness);

      const bioAuth = await getStoredValue<boolean>("biometricAuth");
  ...
  const handleFreshnessChange = async (value: string) => {
    ...
    await storeValue("dataFreshness", value);
  };
